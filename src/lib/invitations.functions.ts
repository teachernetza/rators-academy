...
export const sendInvitation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await (await db()).from("invitations").upsert({
...
export const respondInvitation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.accept) {
      if (inv.role === "student") {
        await (await db()).from("enrollments").upsert(
          { student_id: inv.invitee_id, course_id: inv.course_id },
          { onConflict: "student_id,course_id" }
        );
      } else if (inv.role === "teacher") {
        await (await db()).from("courses").update({ teacher_id: inv.invitee_id }).eq("id", inv.course_id);
      }
    }
    await (await db()).from("invitations").update({
