...
async function assertTeacherOfSubmission(userId: string, submissionId: string) {
  ...
  if (teacherId !== userId) {
    const { data: prof } = await (await db()).from("profiles").select("role").eq("id", userId).maybeSingle();
...
export const teacherListSubmissions = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: courses } = await (await db()).from("courses").select("id").eq("teacher_id", context.userId);
    const courseIds = (courses ?? []).map((c) => c.id);
    if (!courseIds.length) return [];
    const { data: sections } = await (await db()).from("sections").select("id, course_id").in("course_id", courseIds);
    const sectionIds = (sections ?? []).map((s) => s.id);
    if (!sectionIds.length) return [];
    const { data: lessons } = await (await db()).from("lessons").select("id, title, section_id").in("section_id", sectionIds).eq("type", "activity");
    const lessonIds = (lessons ?? []).map((l) => l.id);
    if (!lessonIds.length) return [];

    let q = (await db()).from("activity_submissions")
    ...
    const { data: students } = studentIds.length
      ? await (await db()).from("profiles").select("id, full_name").in("id", studentIds)
      : { data: [] as any[] };
    ...
    const { data: courseRows } = await (await db()).from("courses").select("id, title").in("id", courseIds);
    ...
    return await Promise.all((subs ?? []).map(async (s) => {
      ...
      if (s.file_url) {
        const { data: signed } = await (await db()).storage.from("submissions").createSignedUrl(s.file_url, 3600);
...
export const teacherGradeSubmission = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await (await db()).from("activity_submissions").update({
      grade: data.grade,
...
export const teacherGetStudentMatrix = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data: courses } = await (await db()).from("courses").select("id, title").eq("teacher_id", context.userId);
    const courseIds = (courses ?? []).map((c) => c.id);
    if (!courseIds.length) return { courses: [], students: [], cells: {} };
    const { data: enrolls } = await (await db()).from("enrollments").select("student_id, course_id, progress").in("course_id", courseIds);
