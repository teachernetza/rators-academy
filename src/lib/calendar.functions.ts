...
export const listUpcomingForStudent = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { data: enrolls } = await (await db())
      .from("enrollments")
      .select("course_id")
    ...
    const { data: lessons } = await (await db())
      .from("lessons")
    ...
    const { data: completions } = await (await db())
...
export const listUpcomingForTeacher = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    let coursesQ = (await db()).from("courses").select("id,title");
